...
{
    Task HandleAsync<T>(ConsumeContext<T> context, CancellationToken cancellationToken = default) where T : class;
}
...
public abstract class SagaOrchestrator<TState> : ISagaOrchestrator where TState : SagaState
{
    ...
    protected abstract Task TransitionAsync<T>(TState state, ConsumeContext<T> context, CancellationToken cancellationToken) where T : class;
}
